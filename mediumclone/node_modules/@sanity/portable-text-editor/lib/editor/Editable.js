...
var PortableTextEditable = /*#__PURE__*/(0, _react.forwardRef)(function PortableTextEditable(props, forwardedRef) {
  ...
  var initialValue = (0, _react.useMemo)(() => (0, _values.toSlateValue)(getValueOrInitialValue(value, [placeHolderBlock]), blockType.name, _weakMaps.KEY_TO_SLATE_ELEMENT.get(slateEditor)), // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  (0, _react.useMemo)(() => withHotKeys(withInsertData(withEditableAPI((0, _slateReact.withReact)(slateEditor)))), [slateEditor, withEditableAPI, withHotKeys, withInsertData]); // Track selection (action) state
  ...
  }, [keyGenerator, portableTextFeatures, renderAnnotation, renderChild, renderDecorator, renderPlaceholder, readOnly]); // Restore value from props
  ...
  (0, _react.useEffect)(() => {
    ...
    if (fromMap !== slateEditor.children) {
      ...
      var slateValueFromProps = (0, _values.toSlateValue)(getValueOrInitialValue(value, defaultValue), blockType.name, _weakMaps.KEY_TO_SLATE_ELEMENT.get(slateEditor));
      var val = value || defaultValue;
      val.forEach((blk, index) => {
        if (slateEditor.isTextBlock(blk)) {
          if (!(0, _isEqual2.default)((0, _values.toSlateValue)([blk], blockType.name)[0], slateEditor.children[index])) {
            equal = false;
          }
        } else {
  ...
  }, [change$, isSelecting, isThrottling, placeHolderBlock, blockType.name, slateEditor, value]); // Restore selection from props
